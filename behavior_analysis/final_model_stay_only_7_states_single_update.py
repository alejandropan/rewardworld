...
def generate_data_stay(data, all_contrasts, learning_rate=0.3, beliefSTD=0.1, 
	...
	for t in range(len(data[0])):
		...
		Q_L, Q_R = simulation_QL_QR(Q, perceived_contrast, all_contrasts, beliefSTD)
		if t == 0:
		    (l_stay, r_stay) = [0,0]
		else:
		    previous_choice= [0,0]
...
boxplot_model_parameters_per_mouse(model_parameters, 
                                       model_type= 'w_stay', 
                                       save = True)
plot_q_trial_whole_dataset(psy, save= False)
...
simulate_and_plot(modelled_data, model_parameters, 
                    model_type= 'w_stay')
...
for mouse in mice:
    ...
    model_choice_raw_prob(psy, mouse, save = True)
