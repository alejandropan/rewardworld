...
def simulate_and_plot(modelled_data, model_parameters, 
    ...
    for mouse in mice:
        ...
        sim_data = sim_data.rename(columns={0: "rewards", 1: "signed_contrast",
                                            2: "simulated_choices", 3: "model_laser"})
...
def calculate_QL_QR(modelled_data, model_parameters, 
    ...
    for mouse in mice:
        ...
        if model_type == 'standard':
            ...
            pregen_all_posteriors=False, accu=True, retrieve_Q=True)
...
def session_neg_log_likelihood_stay(params, *data, pregen_all_posteriors=False, accu=False, retrieve_Q =  False):
...
def optimizer_stay(data, num_fits =20, initial_guess=[0.1, 1, 0, 1, 1], nphr = False):
	...
	for i in range(num_fits):
		...
		if i != 0:
			...
			if nphr == False:
				extraVal_guess = np.random.uniform(0.01,2)
			else:
				extraVal_guess = np.random.uniform(-2,-0.01)
...
def generate_data_stay(data, all_contrasts, learning_rate=0.3, 
                       ...
                       stay = 1, is_verbose=False, propagate_errors = False):
...
if __name__ == '__main__':
    ...
    for i, mouse in enumerate(mice[1:]): 
        ...
        cv_aic_stay = aic((session_neg_log_likelihood_stay(best_x_stay,
                  *data_test, pregen_all_posteriors=False))*-1,5)
        
       
        cv_LL_stay = (session_neg_log_likelihood_stay(best_x_stay, *data_test,
                                                      pregen_all_posteriors=False))*-1
