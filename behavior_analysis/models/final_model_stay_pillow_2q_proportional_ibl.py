...
def session_neg_log_likelihood_stay(params, *data, pregen_all_posteriors=True,
    ...
    all_contrasts = np.array([-1,-0.5,-0.25, -0.125, -0.0625, 0,\
                                  0.0625, 0.125, 0.25, 0.5, 1])
...
all_contrasts = np.array([-1,-0.5,-0.25, -0.125, -0.0625,0, 0,\
                                  0.0625, 0.125, 0.25, 0.5, 1])
num_contrasts = len(all_contrasts)
...
for i, mouse in enumerate(mice):
        ...
        model_parameters_mouse['model_name'] = 'armin real'
...
def generate_data_stay(data, all_contrasts, learning_rate=0.3,
    ...
    for t in range(len(data[0])):
        ...
        Q[choice] +=  learning_rate * (reward - Q_chosen) * contrast_posterior[choice]
        
    return rewards, true_contrasts, choices, lasers
# Simulate
psy_sim = pd.DataFrame()
for i, mouse in enumerate(mice):
        model_data_nphr, simulate_data_nphr  = \
            ibl_df_to_Q_learning_model_format(psy.loc[psy['mouse_name'] == mouse],
                                              virus = psy.loc[psy['mouse_name'] == mouse, 'virus'].unique()[0])
        ...
        best_x_stay = model_parameters.loc[model_parameters['mouse']==mouse, 'x'][0]
        ...
        for i in range(10): # 10 simulations
            ...
            sim_data = sim_data.rename(columns={0: "model_rewards", 
                                                ...
                                                2: "simulated_choices",
                                                3: "model_laser"})
            sim_data['mouse'] = mouse
            psy_sim = pd.concat([psy_sim, sim_data])
...
# Fit simulation
model_parameters_sim = pd.DataFrame()
psy_sim = psy.copy()
psy_sim.drop(columns=['feedbackType', 'choice'], inplace=True)
psy_sim = psy_sim.rename(columns={"model_rewards":'feedbackType', 
                                  "simulated_choices": 'choice'})
# Fitting and calculating
for i, mouse in enumerate(mice):
    
        # Transform the data
        model_data_nphr, simulate_data_nphr  = \
            ibl_df_to_Q_learning_model_format(psy_sim.loc[psy_sim['mouse_name'] == mouse],
                                              virus = psy_sim.loc[psy_sim['mouse_name'] == mouse, 'virus'].unique()[0])
        # Obtain sessions switches
        session_switches = np.zeros(len(model_data_nphr))
        for session in model_data_nphr['ses'].unique():
             session_switches[model_data_nphr.ses.ge(session).idxmax()]=1
        ...
        choices = model_data_nphr['choice'].to_numpy().astype(int)
        contrasts = model_data_nphr['stimTrials'].to_numpy()
        rewards = model_data_nphr['reward'].to_numpy().astype(int)
        ...
        model_parameters_mouse['LL'] = (LL/len(data_test[0])) # Log likelihood per trial
        model_parameters_mouse['accu'] = cv_acc_stay
        model_parameters_mouse['model_name'] = 'Armin simulation'
        model_parameters_mouse['mouse'] = mouse
        model_parameters_mouse['virus'] = psy.loc[psy['mouse_name'] == mouse, 'virus'].unique()[0]
        model_parameters_sim = pd.concat([model_parameters_sim, model_parameters_mouse])
        
# Compare parameters
model_parameters_sim['model_name']='from_simulation'
model_parameters['model_name'] = 'from_real_data'
...
def boxplot_model_parameters_per_mouse(model_parameters,
    ...
    fig, ax =  plt.subplots()
    sns.set(style='white')
    model = model_parameters['x']
    params = [r'$\alpha$', r'$\sigma$',
              r'$\tau$', r'$\gamma$', r'$\phi$']
    mod = pd.DataFrame(columns = ['params'])
    for i in range(len(model)):
        temp_mod = pd.DataFrame(model.iloc[i])
        temp_mod['params'] = params[:len(temp_mod)]
        mod = mod.append(temp_mod)
    sns.swarmplot(data = mod,  x = 'params', y = 0, split = False)
...
boxplot_model_parameters_per_mouse(model_parameters_sim)

plt.subplots(2,plt)
sns.lineplot(data = psy_sim,x='signed_contrast', y='choice' )
sns.lineplot(data = psy,x='signed_contrast', y=1*((psy['choice']*-1)>0))
plt.xlabel('Fraction Choice Right')

boxplot_model_parameters_per_mouse(model_parameters1)
plt.ylabel('Diff Coef (Real_Data - Sim_Data)')