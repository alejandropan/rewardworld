...
def trial_log_likelihood_stay(params, trial_data, Q, all_contrasts, all_posteriors, 
    ...
    x = 1
    learning_rate, extraVal, beta = params
    Q =  Q.copy()
...
def optimizer_stay(data, num_fits = 10, initial_guess=[0.1, 1, 0, 1]):
    ...
    for i in range(num_fits):
        ...
        if i != 0:
            ...
            initial_guess = [lr_guess, extraVal_guess, beta_guess]

        # Run the fit
        res = so.minimize(session_neg_log_likelihood_stay, initial_guess, args=data, 
                    method='L-BFGS-B', bounds=[(0, 2), (-2, 2), (0.01, 5)])
