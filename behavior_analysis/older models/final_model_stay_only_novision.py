            ...
            if os.path.exists(mouse_path):
                ...
                for d in dates:
                    ...
                    for ses in sessions:
                        ...
                        date = d[-10:]
                        Q_files = ['BlindQL', 'BlindQR', 'BlindQL0', 'BlindQR0', 'BlindITIQL', 'BlindITIQR', 
                                   'BlindITIQL0', 'BlindITIQR0']
                        for Q in Q_files:
...
def optimizer_stay(data, num_fits = 4, initial_guess=[0.1, 0, 1, 1]):
	...
	for i in range(num_fits):
		...
		if i != 0:
			...
			extraVal_guess = np.random.uniform(-10,10)
			beta_guess = np.random.uniform(0.01, 10)
		...
		res = so.minimize(session_neg_log_likelihood_stay, initial_guess, args=data, 
                    method='L-BFGS-B', bounds=[(0, 2), (-10, 10), (0.01, 10), 
                                    (-1,1)])
...
# Accuracy by block
psy['model_correct'] = 0
psy.loc[psy['choice']==0,'model_correct'] = 0
psy.loc[psy['choice']==-1,'model_correct'] = \
    (psy.loc[psy['choice']==-1,'pRight']>0.5)*1
psy.loc[psy['choice']==1,'model_correct'] = \
    (psy.loc[psy['choice']==1,'pRight']<0.5)*1
...
psy['model_correct_n_repeat'] = 0
psy['model_correct_n_repeat'] = \
    (psy['repeat'] == psy['model_correct'])*1 


model_n_repeated_mouse = psy.groupby(['mouse_name']).mean()['model_correct_n_repeat']
repeated_byblock_mouse = psy.groupby(['opto_block','mouse_name']).mean()['repeat']
repeated_mouse = psy.groupby(['mouse_name']).mean()['repeat']
accuracy_block_mouse = psy.groupby(['opto_block','mouse_name']).mean()['model_correct']
accuracy_mouse = psy.groupby(['mouse_name']).mean()['model_correct']
