...
def extract_behaviour_sync(sync, chmap=None, display=False, bpod_trials=None, tmax=np.inf):
    ...
    #Patch
    t_feedback = np.zeros(len(bpod_trials['feedback_times']))
    t_feedback[bpod_trials['feedbackType']==1] = t_valve_open
    t_feedback[bpod_trials['feedbackType']==-1] = t_iti_in

    t_tone_in, t_error_tone_in = _assign_events_audio(
        audio['times'], audio['polarities'])

    t_laser_cue_in = []
    for sound in t_tone_in:
        for r in t_valve_open:
            if (sound>(r-0.05)) & (sound<(r+0.05)):
                t_laser_cue_in.append(sound)
    t_laser_cue_in = np.array(t_laser_cue_in)
    t_ready_tone_in =  np.setdiff1d(t_tone_in, t_laser_cue_in)

    assert len(t_ready_tone_in) == len(bpod_trials['feedbackType'])
    t_trial_start = np.zeros(len(t_ready_tone_in)+1)
    t_trial_start[np.where(bpod_trials['feedbackType']==1)[0]+1] = \
        t_valve_open + 4
    t_trial_start[np.where(bpod_trials['feedbackType']==-1)[0]+1] = \
        t_iti_in + 4
    ...
    if bpod_trials:
        bpod_feed = bpod_trials['feedback_times'][bpod_trials['feedbackType']==1]
        fcn, drift, ibpod, ifpga = dsp.utils.sync_timestamps(
            bpod_feed, t_valve_open, return_indices=True)
        # if it's drifting too much
        if drift > 200 and bpod_feed.size != t_valve_open.size:
            raise err.SyncBpodFpgaException("sync cluster f*ck")
        missing_bpod = fcn(bpod_feed[np.setxor1d(ibpod, np.arange(len(bpod_feed)))])
        t_valve_open = np.sort(np.r_[t_valve_open, missing_bpod])
    else:
...
def _assign_events_bpod(bpod_t, bpod_polarities, ignore_first_valve=True):
    ...
    i_valve_open = np.where(np.logical_and(dt > 0.1,
                                           dt < ITI_TTL_LEN))[0] * 2
...
class FpgaTrials(BaseExtractor):
    save_names = (
                  ('_ibl_trials.feedbackType.npy', '_ibl_trials.choice.npy',
                   ...
                   '_ibl_trials.goCueTrigger_times.npy', None, None, '_ibl_trials.stimFreezeTrigger_times.npy', None, None,
...
def sorting_sync_and_alf(session_path, overwrite=False):
    ...
    for ap_file, label in ap_files:
            ks2_dir = session_path.joinpath("spike_sorters", "pykilosort", label)
            probe_out_path = session_path.joinpath("alf", label,"pykilosort")
            ...
            tar_dir = session_path.joinpath('spike_sorters', 'pykilosort', label)
            tar_dir.mkdir(parents=True, exist_ok=True)
            out = spikes.ks2_to_tar(ks2_dir, tar_dir)
            out_files.extend(out)

def move_phy_files(ses):
    ks2 = ses+'/spike_sorters/pykilosort'
    probes = os.listdir(ks2)
    items = ['pc_feature_ind.npy','pc_features.npy',
    'cluster_Amplitude.tsv','cluster_ContamPct.tsv', 'cluster_group.tsv'
    #'cluster_info.tsv'
    ]
    for item in items:
        try:
...
if __name__ == "__main__":
    ses = sys.argv[1]
    session_path = Path(ses)
    syncs, out_files = ephys_fpga.extract_sync(session_path, overwrite=True)
    extract_all(session_path, save=True, bin_exists=False)
