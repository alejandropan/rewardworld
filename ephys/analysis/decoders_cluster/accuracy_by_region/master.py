...
np.save('/jukebox/witten/Alex/decoders_residuals_results/accuracy_by_region/areas_summary.npy', AREAS)
...
for ses_n,ses in enumerate(SESSIONS):
    for area_n, area in enumerate(AREAS[ses_n]):
        ...
        counter+=1
id_dict.to_csv('/jukebox/witten/Alex/decoders_residuals_results/accuracy_by_region/id_dict.csv') # Translate slurm ids to regions

# counter-1 since last counter is not used in dict
print(subprocess.run(['sbatch --array=0-'+str(counter-1)+' /jukebox/witten/Alex/PYTHON/rewardworld/ephys/analysis/decoders_cluster/accuracy_by_region/decoder_job_accu_by_region.cmd'], shell=True))
