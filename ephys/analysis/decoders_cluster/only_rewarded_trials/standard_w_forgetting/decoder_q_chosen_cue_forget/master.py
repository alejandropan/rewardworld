...
np.save('/jukebox/witten/Alex/decoders_rewarded_results/decoder_output_qchosen_cue_forget/areas_summary.npy', AREAS)
...
for ses_n,ses in enumerate(SESSIONS):
    for area_n, area in enumerate(AREAS[ses_n]):
        ...
        counter+=1
id_dict.to_csv('/jukebox/witten/Alex/decoders_rewarded_results/decoder_output_qchosen_cue_forget/id_dict.csv') # Translate slurm ids to regions
# counter-1 since last counter is not used in dict
print(subprocess.run(['sbatch --array=0-'+str(counter-1)+' /jukebox/witten/Alex/PYTHON/rewardworld/ephys/analysis/decoders_cluster/only_rewarded_trials/standard_w_forgetting/decoder_q_chosen_cue_forget/decoder_job_q_chosen_cue_forget.cmd'], shell=True))