...
def load_decoders(decoder_path, var = None, epoch = None, x_type = None):
    ...
    files = []
    f=[]
    for i in os.listdir(decoder_path):
        if os.path.isfile(os.path.join(decoder_path,i)) and 'real' in i:
            files.append(i)
    for file in files:
...
def plot_null_analysis(decoders_restricted , nsummary_restricted, varss = None, epoch = None):
    ...
    for i, reg in enumerate(selected_regions):
        ...
        sns.lineplot(data=region_data, x='time_bin', y='r', errorbar=('ci', 95), color=pals[reg])
        sns.lineplot(data=region_null, x='time_bin', y='r', errorbar=('ci', 95), color=pals[reg], alpha=0.3)
        plt.axvline(x = 4, ymin = 0, linestyle='--', color='k', alpha=0.5)
        plt.xticks(np.arange((post_time-pre_time)/bin_size)[::5], np.round(np.arange(pre_time,post_time,bin_size)[1::5],2), rotation=90)
