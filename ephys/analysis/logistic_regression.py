...
def add_laser_block_regressors(ses_df):
    ses_df = ses_df.rename(columns={'opto_block': 'laser_block'})
    ses_df['laser']=0
    ses_df.loc[ses_df['laser_block']==1,'laser'] = \
        1*(ses_df.loc[ses_df['laser_block']==1,'outcome'])
    ses_df['previous_choice_1']= ses_df['choice'].shift(1)
    ses_df['previous_choice_2']= ses_df['choice'].shift(2)
    ses_df['previous_choice_3']= ses_df['choice'].shift(3)
    ses_df['previous_choice_4']= ses_df['choice'].shift(4)
    ses_df['previous_choice_5']= ses_df['choice'].shift(5)
    ses_df['previous_choice_6']= ses_df['choice'].shift(6)
    ses_df['previous_choice_7']= ses_df['choice'].shift(7)
    ses_df['previous_choice_8']= ses_df['choice'].shift(8)
    ses_df['previous_choice_9']= ses_df['choice'].shift(9)
    ses_df['previous_choice_10']= ses_df['choice'].shift(10)
...
def fit_GLM_w_laser_10(data, trials_back=10, save_dm=False, drop=True):
    '''
    Parameters
    ----------
    ...
    if drop==True:
        behav =  behav.drop(columns=[ 'outcome', 'left_reward', 'right_reward',
        'probabilityLeft', 'laser_block', 'laser_reward', 'water_reward',
        'goCue_trigger_times', 'stimOn_times', 'response_times', 'QL', 'QR',
        'QLlaser', 'QRlaser', 'QLstay', 'QRstay', 'QLreward', 'QRreward',
        'choice_prediction', 'accuracy', 'mouse', 'date', 'ses',
        'trial_within_block', 'block_number', 'trial_within_block_real',
        'probabilityLeft_next', 'opto_block_next', 'probabilityLeft_past',
        'opto_block_past', 'block_change', 'block_number_real', 'choice_1',
        'transition_analysis', 'transition_type', 'transition_analysis_real',
        'transition_type_real'])
