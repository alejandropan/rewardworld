...
def load_neural_data(probe_folder, histology = True):
    '''
    Parameters
    ----------
    probe_folder : folder of probe (str)

    Returns
    -------
    arrays with cluster measures
    '''
        clusters_depths = np.load(probe_folder + '/clusters.depths.npy')
        cluster_metrics = pd.read_csv(probe_folder + '/clusters.metrics.csv')
        spikeclusters = np.load(probe_folder + '/spikes.clusters.npy')
        spiketimes = np.load(probe_folder + '/spikes.times.npy')
        good_clusters = cluster_metrics.loc[cluster_metrics['ks2_label']=='good', 'id']
        cluster_channels = np.load(probe_folder + '/clusters.channels.npy')
        if histology == True:
            allen = np.load(probe_folder + '/channels.locations.npy', allow_pickle=True)
