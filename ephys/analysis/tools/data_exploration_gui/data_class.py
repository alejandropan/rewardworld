...
class DataGroup:
    ...
    def find_files(self):
        ...
        if self.probe_path.parent.name!='alf':
            self.alf_path = self.folder_path.parent.parent
        else:
            self.alf_path = self.folder_path.parent
        dir_path = self.alf_path.parent
    ...
    def load_data(self):
        ...
        try:
            chan_locations = np.load(self.probe_path.joinpath('channels.locations.npy'), allow_pickle=True)
    ...
    def prepare_data(self, spikes, clusters, trials):
        ...
        self.colours[np.where(self.colours != 'good')[0]] = QtGui.QColor('#fdc086') # default for noise and nan
        self.colours[np.where(self.colours == 'good')[0]] = QtGui.QColor('#7fc97f')
        self.locations = clusters.locations
        _, self.depths, self.nspikes = compute_cluster_average(spikes.clusters, spikes.depths)
        self.depths[np.where(np.isnan(self.depths))[0]] = self.clusters.depths[np.where(np.isnan(self.depths))[0]] # use channel depth as default
    ...
    def sort_data(self, order):
        ...
        self.clust_loc = self.locations[order]

        return self.clust_ids, self.clust_amps, self.clust_depths, self.clust_colours, self.clust_loc
