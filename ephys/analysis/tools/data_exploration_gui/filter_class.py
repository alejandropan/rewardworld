...
class FilterGroup:
    ...
    def __init__(self):
        ...
        self.reward_options_text = QtWidgets.QLabel('Opto Block')
        self.reward = [0, 1]
        self.reward_options = QtWidgets.QListWidget()
        for val in self.reward:
            item = QtWidgets.QListWidgetItem(str(val))
            item.setFlags(item.flags() | QtCore.Qt.ItemIsUserCheckable)
            item.setCheckState(QtCore.Qt.Checked)
            self.reward_options.addItem(item)
        ...
        filter_options = ['all', 'correct', 'incorrect', 'left', 'right',
    ...
    def group_filter_widget(self):
        ...
        group_layout.addWidget(self.reward_options_text)
        group_layout.addWidget(self.reward_options)
    ...
    def get_checked_reward(self):

        '''
        Finds the reward options that are selected. Called by on_reward_list_changed in gui_main.

        Returns
        ----------

        reward: list
            A list of the reward options that are selected

        '''

        reward = []
        for idx in range(self.reward_options.count()):
            if self.reward_options.item(idx).checkState() == QtCore.Qt.Checked:
                reward.append(self.reward[idx])

        return reward


    def compute_and_sort_trials(self, reward):
        #Precompute trials for a given reward set
        #All
        all_trials = Bunch()
        ...
        idx= np.empty(0, int)
        for r in reward:
                idx = np.where(self.trials['opto_block'] == r)[0]
        ...
        idx= np.empty(0, int)
        for r in reward:
            idx = np.where((self.trials['opto_block'] == r) & (self.trials['feedbackType'] == 1))[0]
            trials_id = np.append(trials_id, idx)
        trials_id = np.setdiff1d(trials_id, self.nan_trials)
        ...
        idx= np.empty(0, int)
        for r in reward:
            idx = np.where((self.trials['opto_block'] == r) & (self.trials['choice'] == -1))[0]
            trials_id = np.append(trials_id, idx)
        trials_id = np.setdiff1d(trials_id, self.nan_trials)
        ...
        idx= np.empty(0, int)
        for r in reward:
            idx = np.where((self.trials['opto_block'] == r) & (self.trials['feedbackType'] == 1)
                    & (self.trials['choice'] == -1))[0]
            trials_id = np.append(trials_id, idx)
        trials_id = np.setdiff1d(trials_id, self.nan_trials)
    ...
    def compute_trial_options(self, trials):
        ...
        self.right_idx = np.setdiff1d(np.where(self.trials['choice'] == 1)[0], self.nan_trials)
        self.left_idx = np.setdiff1d(np.where(self.trials['choice'] == -1)[0], self.nan_trials)
    ...
    def reset_filters(self, stim = True):
        ...
        if stim is True:
            for idx in range(self.reward_options.count()):
                item = self.reward_options.item(idx)
                item.setCheckState(QtCore.Qt.Checked)
        ...
        return reward, case, sort_method
