...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.filter.reward_options.itemClicked.connect(self.on_reward_list_changed)
    ...
    def reset_gui(self):
        ...
        [self.reward, self.case, self.sort_method] = \
    ...
    def initialise_gui(self):
        ...
        self.scatter.populate(self.clust_amps, self.clust_depths, self.clust_ids,
                              self.clust_colours, self.clust_loc)
    ...
    def on_folder_button_clicked(self):
        ...
        if str(folder_path) == '.':
            ...
            [self.clust_ids, self.clust_amps, self.clust_depths, self.clust_colours, self.clust_loc] = \
            ...
            self.trials = self.filter.compute_and_sort_trials(self.reward)
    ...
    def on_reward_list_changed(self):

        self.reward = self.filter.get_checked_reward()
        self.trials = self.filter.compute_and_sort_trials(self.reward)
        self.plots.change_all_plots_final(self.data, self.clust, self.trials, self.case, self.sort_method, self.hold)
        self.check_n_trials()
        self.update_display_string()
    ...
    def on_reset_filter_button_clicked(self):
        ...
        [self.reward, self.case, self.sort_method] = self.filter.reset_filters()
        self.trials = self.filter.compute_and_sort_trials(self.reward)
        self.plots.change_all_plots_final(self.data, self.clust, self.trials, self.case, self.sort_method, self.hold)
        self.update_display_string()
