...
from spikeglx import read_meta_data
import spikeglx
...
def version3B(ses_path, display=True, type=None, tol=2.5, probe_names=None):
    ...
    for ef in ephys_files:
        ...
        if sync_nidq.times.size != sync_probe.times.size:
            ...
            nidq = read_meta_data(glob(ses_path.as_posix() + '/raw_ephys_data/*.nidq.meta')[0])['firstSample']
            probe = read_meta_data(glob(ses_path.as_posix() + '/raw_ephys_data/probe00/*.ap.meta')[0])['firstSample']
            nidq_after = (nidq>probe)
        ...
        if nidq_after==True:
            ...
            sync_idx = abs(sync_nidq.times.size- sync_probe.times.size)
            timestamps, qc = sync_probe_front_times(sync_probe.times, sync_nidq.times[sync_idx:], sr,
                                                    display=display, type=type_probe, tol=tol)            
        qc_all &= qc
