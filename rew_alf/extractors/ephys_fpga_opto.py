...
def _laser_event_extraction(laser_fronts_t, laser_fronts_p):
    """
    From detected fronts on the laser sync traces, outputs the synchronisation events
    related to individual pulses and entire laser epoch
    :param laser_fronts_t: numpy vector containing times of fronts
    :param laser_fronts_p: numpy vector containing polarity of fronts (1 rise, -1 fall)
    :return: numpy arrays of times t_laser_start, t_laser_end and t_epoch_start, t_epoch_end
    """
    # make sure that there are no 2 consecutive fall or consecutive rise events
    assert(np.all(np.abs(np.diff(laser_fronts_p)) == 2))
    # make sure that the first event is a rise
    assert(laser_fronts_p[0] == 1)
    dt = np.diff(laser_fronts_t)[::2]
    t_laser_start = laser_fronts_t[laser_fronts_p == 1]
    t_laser_end = laser_fronts_t[laser_fronts_p == -1]
    t_epoch_start = t_laser_start[0]
    t_epoch_start = np.append(t_epoch_start, laser_fronts_t[np.where(np.diff(laser_fronts_t)>1.5)[0] +1])
    t_epoch_end = laser_fronts_t[np.where(np.diff(laser_fronts_t)>1.5)]
    t_epoch_end = np.append(t_epoch_end, t_laser_end[-1])
    
    return t_laser_start, t_laser_end, t_epoch_start, t_epoch_end
...
def extract_behaviour_sync(sync, output_path=None, save=False, chmap=None, display=False,
    ...
    #@alejandro extract opto
    opto = _get_sync_fronts(sync, chmap['opto'], tmax=tmax)
    ...
    #@alejandro extract opto
    t_laser_start, t_laser_end, t_epoch_start, t_epoch_end = _laser_event_extraction(
        opto['times'], opto['polarities'])
    ...
    #opto assertions
    assert max(t_epoch_end - t_epoch_start) < 1.1
    assert min(np.diff(t_laser_start)) > 0.049
    
    # stimOn_times: first fram2ttl change after trial start
    trials = Bunch({
        'laser_epoch_in': _assign_events_to_trial(t_trial_start, t_epoch_start),
        'laser_epoch_out': _assign_events_to_trial(t_trial_start, t_epoch_end),
    ...
    if save and output_path:
        output_path = Path(output_path)
        np.save(output_path / '_ibl_trials.laser_epoch_in.npy', trials['laser_epoch_in'])
        np.save(output_path / '_ibl_trials.laser_epoch_out.npy', trials['laser_epoch_out'])
