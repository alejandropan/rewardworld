...
def get_psych_data(tph):
    ...
    prop_resp_ccw00 = get_prop_ccw_resp(0.0)
    prop_resp_ccw01 = get_prop_ccw_resp(0.1)

    prop_resp_ccw02 = get_prop_ccw_resp(0.2)

    prop_resp_ccw05 = get_prop_ccw_resp(0.5)

    prop_resp_ccw06 = get_prop_ccw_resp(0.6)

    prop_resp_ccw07 = get_prop_ccw_resp(0.7)
    prop_resp_ccw08 = get_prop_ccw_resp(0.8)

    return sig_contrasts_all, prop_resp_ccw00, prop_resp_ccw01, prop_resp_ccw02, prop_resp_ccw05, prop_resp_ccw06, prop_resp_ccw07, prop_resp_ccw08
...
def get_chron_data(tph):
    ...
    rts00,rts01, rts02, rts05, rts06, rts07, rts08 = get_rts(0.0),get_rts(0.1),get_rts(0.2), get_rts(0.5), get_rts(0.6), get_rts(0.7), get_rts(0.8)

    return sig_contrasts_all, rts00,rts01, rts02, rts05, rts06, rts07, rts08
...
def plot_bars(bar_data, ax=None):
    ...
    elif bar_data["stim_pl"] == 0.0:

        clr = "red"

    elif bar_data["stim_pl"] == 0.1:

        clr = "yellow"

    elif bar_data["stim_pl"] == 0.6:

        clr = "cyan"

    elif bar_data["stim_pl"] == 0.7:
...
def plot_psych(psych_data, ax=None):
    ...
    ax.plot(x, y00, c="r", label="CCW responses 00/60", marker="o", ls="-", alpha=0.5)

    ax.plot(x, y01, c="y", label="CCW responses 10/70", marker="o", ls="-")

    ax.plot(x, y06, c="cyan", label="CCW responses 60/00", marker="o", ls="-")
    ax.plot(x, y07, c="orange", label="CCW responses 70/10", marker="o", ls="-")   
