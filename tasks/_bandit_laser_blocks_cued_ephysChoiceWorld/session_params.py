...
class SessionParamHandler(object):
    ...
    def __init__(self, task_settings, user_settings, debug=False, fmake=True):
        ...
        self.LASER_TONE_DURATION = float(self.LASER_TONE_DURATION)
        self.LASER_TONE_FREQUENCY = float(self.LASER_TONE_FREQUENCY)
        self.LASER_TONE_AMPLITUDE = float(self.LASER_TONE_AMPLITUDE)
        ...
        self.GO_TONE = None
        self.LASER_TONE = None
        self = sound.init_sounds(self)  # sets GO_TONE and LASER_TONE
        ...
        if self.SOFT_SOUND is None:
            sound.configure_sound_card(
                sounds=[self.GO_TONE, self.LASER_TONE],
                indexes=[self.GO_TONE_IDX, self.LASER_TONE_IDX],
                sample_rate=self.SOUND_SAMPLE_FREQ,
            )

        self.OUT_TONE = ("SoftCode", 1) if self.SOFT_SOUND else ("Serial3", 6)
        self.OUT_LASER_TONE = ("SoftCode", 2) if self.SOFT_SOUND else ("Serial3", 7)
        self.OUT_STOP_SOUND = (
    ...
    def play_noise(self):
        self.SD.play(self.LASER_TONE, self.SOUND_SAMPLE_FREQ)
    ...
    def reprJSON(self):
        ...
        d["LASER_TONE"] = "laser_tone(freq=-1, dur={}, amp={})".format(
            self.LASER_TONE_FREQUENCY, self.LASER_TONE_DURATION, self.LASER_TONE_AMPLITUDE
        )
        d["SD"] = str(d["SD"])
