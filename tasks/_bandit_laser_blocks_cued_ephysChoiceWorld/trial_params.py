...
def update_block_params(tph):
    ...
    low_press = (tph.stim_probability_left>0.5)*1
    left_right_total = [len(np.where(last_10==1)[0]),len(np.where(last_10==-1)[0])]
...
class TrialParamHandler(object):
    ...
    def __init__(self, sph):
        ...
        self.out_laser = sph.OUT_LASER_TONE
        ...
        self.choice_right = self.threshold_events_dict[sph.CHOICE_THRESHOLDS[0]]
...
REWARD LEFT:          {self.left_reward}

REWARD RIGHT:         {self.right_reward}
...
##########################################"""
    ...
    def trial_completed(self, behavior_data):
        ...
        if ('RotaryEncoder1_1' in self.behavior_data['Events timestamps']) & ('RotaryEncoder1_2' in self.behavior_data['Events timestamps']):

            left_time = self.behavior_data['Events timestamps']['RotaryEncoder1_2'][0]

            right_time = self.behavior_data['Events timestamps']['RotaryEncoder1_1'][0]
        ...
        elif 'RotaryEncoder1_1' in self.behavior_data['Events timestamps']:

            self.response_side_buffer.append(-1)

        elif 'RotaryEncoder1_2' in self.behavior_data['Events timestamps']:
        ...
        params["left_reward"] = int(params["left_reward"])

        params["right_reward"] = int(params["right_reward"])
