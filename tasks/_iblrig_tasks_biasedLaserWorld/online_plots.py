...
def get_barplot_data(tph):
    ...
    out['stim_pl'] = tph.opto_probability_left
    return out
...
def get_psych_data(tph):
    ...
    opto_probability_left_buffer = np.array(tph.opto_probability_left_buffer)

    def get_prop_ccw_resp(stim_prob_left):
        ntrials_ccw = np.array([
            sum(response_side_buffer[(
                opto_probability_left_buffer == stim_prob_left) & (
